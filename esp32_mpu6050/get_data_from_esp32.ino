    ...
    <html>
      ...
      <body>
        ...
        <div style="text-align:center;">
          <button class="button-activity" onclick="startRecording('walking')" id="walkingBtn">Walking</button>
          <button class="button-activity" onclick="startRecording('jogging')" id="joggingBtn">Jogging</button>
          <button class="button-activity" onclick="startRecording('standing')" id="standingBtn">Standing</button>
          <button class="button-activity" onclick="startRecording('sitting')" id="sittingBtn">Sitting</button>
          <button class="button-activity" onclick="startRecording('falling')" id="fallingBtn">Falling</button>
          <button id="recordButton" style="display:none;" onclick="stopRecording()">Stop Recording</button> <br><br>
          <span id="activityLabelDisplay">Current Activity: Unknown</span>
        </div>

        <a href="/download" id="downloadLink" style="display:none;">Download CSV</a>
        <div id="sensorData">
        ...
        <script>
          ...
          function startRecording(activity) {
            if (!isRecordingNow) {
              ...
              activityLabelDisplay.innerText = 'Current Activity: ' + activity.charAt(0).toUpperCase() + activity.slice(1);
              ...
              fetch('/toggleRecord', {method: 'POST'})
                ...
                .then(state => {
                  if (state === 'false') {
                    ...
                    activityLabelDisplay.innerText = 'Current Activity: Unknown';
            ...
            fetch('/sensorData')
              ...
              .then(data => {
                document.getElementById('sensorData').innerHTML = `
                  <h2>Live Sensor Data</h2>
                  <p>Time: ${data.time}</p>
                  <p>Activity: ${currentActivity.charAt(0).toUpperCase() + currentActivity.slice(1)}</p>
